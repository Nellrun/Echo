"""
Pure aggregation for ``playtime.taste_profile`` — long-term signals.

Mirrors :mod:`echo.providers.gaming.taste` in window shape and rationale
(see that module for the "why disjoint windows" argument). Cross-platform:
the same classification runs over PSN/Steam/Nintendo sessions together,
ranked by **hours played** rather than session count.
"""
...
def build(sessions: Iterable, *, now: datetime | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": (
            ...
            "Cross-platform (PSN/Steam/Nintendo); ranked by total hours, "
            "not session count."
        ),
    }
